...
def create_project(name, video_path):
    c = _conn()
    cur = c.execute("INSERT INTO projects (name,video_path,status) VALUES (?,?,'transcrevendo')",
                    (name, video_path))
    pid = cur.lastrowid; c.commit(); c.close(); return pid

def update_project_status(pid, status):
    c = _conn(); c.execute("UPDATE projects SET status=? WHERE id=?", (status,pid)); c.commit(); c.close()
...
def list_projects():
    c = _conn(); rows = c.execute("SELECT * FROM projects ORDER BY created_at DESC").fetchall(); c.close()
    return [dict(r) for r in rows]

def save_transcription(pid, full_text, segments):
    c = _conn()
    c.execute("INSERT INTO transcriptions (project_id,full_text,segments) VALUES (?,?,?)",
              (pid, full_text, json.dumps(segments, ensure_ascii=False)))
    c.commit(); c.close()

def get_transcription(pid):
    c = _conn()
    r = c.execute("SELECT * FROM transcriptions WHERE project_id=? ORDER BY id DESC LIMIT 1",
                  (pid,)).fetchone(); c.close()
...
def get_cuts(pid, status=None):
    c = _conn()
    if status:
        rows = c.execute("SELECT * FROM cuts WHERE project_id=? AND status=? ORDER BY cut_index",
                         (pid, status)).fetchall()
    else:
        rows = c.execute("SELECT * FROM cuts WHERE project_id=? ORDER BY cut_index",
                         (pid,)).fetchall()
    c.close()
    result = []
    for r in rows:
...
def update_cut_status(cut_id, status):
    c = _conn(); c.execute("UPDATE cuts SET status=? WHERE id=?", (status, cut_id)); c.commit(); c.close()

def update_cut_output(cut_id, paths):
    c = _conn()
    c.execute("UPDATE cuts SET output_paths=?,status='renderizado' WHERE id=?",
              (json.dumps(paths), cut_id)); c.commit(); c.close()
