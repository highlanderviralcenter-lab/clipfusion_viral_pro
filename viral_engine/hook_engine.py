...
class ViralHookEngine:
    ...
    def generate(self, tema: str, nicho: str, platform: str,
                 archetype_id: str = None) -> dict:
        ...
        hook_base = template.format(
            ...
            numero="milhares de", esforco="muito",
            injustica="não consegue avançar",
        )

        enhanced = self.secondary.dual_hook(
            hook_base, nicho, audience["grupo_secundario"])
        final = PlatformOptimizer.optimize(enhanced, platform)

        return {
            "gancho_final":    final,
            "gancho_original": hook_base,
            "archetype":       archetype,
            "publico":         perfil,
            "expansao":        audience["grupo_secundario"],
            "timing":          audience["timing_otimo"],
            "hashtags":        audience["hashtags_sugeridas"],
            "cta":             archetype["cta"],
            "cores":           archetype["cores"],
            "musica":          archetype["musica"],
            "duracao":         archetype["duracao_ideal"],
        }

    def generate_prompt_for_external_ai(self, tema: str, nicho: str,
                                         platform: str, segments: list,
                                         duration: float) -> str:
        """Monta o prompt completo para a IA externa analisar o vídeo."""
        from core.prompt_builder import build_analysis_prompt
        return build_analysis_prompt(segments, duration,
                                     context=f"Tema: {tema}. Nicho: {nicho}.")
