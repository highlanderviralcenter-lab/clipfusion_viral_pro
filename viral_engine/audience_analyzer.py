...
class AudienceAnalyzer:
    ...
    def __init__(self):
        ...
        # Carrega perfis customizados se existir
        custom = os.path.join(os.path.dirname(__file__),
                              "..", "config", "perfis_nicho.json")
    ...
    def analyze(self, nicho: str, platform: str) -> dict:
        ...
        perfil = next(
            (v for k, v in self.perfis.items()
             if k in nicho_low or nicho_low in k),
            DEFAULT_PERFIL)

        grupo_sec = {
            "nome": "Interessados em produtividade",
            "angulo_gancho": "Como isso afeta sua rotina?",
            "expansao_potencial": "25%",
        }

        timing = {
            "tiktok":    ["18h–22h", "12h–14h"],
            "instagram": ["19h–21h", "11h–13h"],
            "youtube":   ["14h–16h", "19h–21h"],
        }.get(platform, ["18h–22h"])

        return {
            "perfil_primario":  perfil,
            "grupo_secundario": grupo_sec,
            "timing_otimo":     timing,
            "hashtags_sugeridas": [
                f"#{nicho.replace(' ', '')}", "#viral",
                "#dica", "#conteudo", "#shorts",
            ],
        }
