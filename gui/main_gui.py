...
Abas: Projeto | Transcrição | IA Externa | Cortes | Render | Histórico
"""
...
import db
from core.transcriber import transcribe, fmt_time
...
class ClipFusionApp:
    ...
    def __init__(self):
        ...
        self.segments     = []
        self.duration     = 0.0
        self.cut_vars     = {}   # cut_id/idx → BooleanVar
    ...
    def _build_ui(self):
        ...
        s.configure("TNotebook.Tab", background=BG3, foreground=GRY,
    ...
    def _tab_projeto(self):
        ...
        self._lbl(f, "Selecione o vídeo longo e configure as opções.",
        ...
        tk.Entry(r1, textvariable=self.v_name, width=44,
                 bg=BG3, fg=WHT, insertbackground=WHT,
                 relief="flat", font=FNT).pack(side="left", padx=10)

        # Contexto
        self._lbl(f, "Contexto (opcional — ajuda a IA entender o tema):").pack(
        ...
        self.ctx_box.insert("1.0",
            "Ex: Podcast sobre vendas com João Silva — episódio sobre prospecção de clientes.")
        ...
        self._chk(op, "Usar VA-API (Intel HD 520)", self.v_vaapi).pack(anchor="w")
        ...
        self._sep(f)
        self._btn(f, "▶  Iniciar Transcrição", self._start_transcription,
                  GRN, wide=True).pack(padx=30, pady=8)
    ...
    def _tab_transcricao(self):
        ...
        self._lbl(f, "Transcrição com timestamps", font=FNTL).pack(
            anchor="w", padx=30, pady=(20,4))
        self._lbl(f, "Gerada pelo Whisper. Revise se necessário.",
                  color=GRY).pack(anchor="w", padx=30)
        ...
        self._btn(f, "▶  Gerar Prompt para IA  →", self._goto_ia,
                  ACC, wide=True).pack(padx=30, pady=(0,20))
    ...
    def _tab_ia(self):
        ...
        self._lbl(f, "Cole no Claude.ai, ChatGPT ou qualquer IA. "
                  "Traga o JSON de resposta para a caixa abaixo.",
                  color=GRY).pack(anchor="w", padx=30)

        self.box_prompt = scrolledtext.ScrolledText(
            f, height=11, bg=BG3, fg="#a5b4fc",
            font=MONO, relief="flat", insertbackground=WHT)
        self.box_prompt.pack(fill="x", padx=30, pady=(4,14))

        self._lbl(f, "Resposta da IA (cole o JSON aqui):", font=FNTB).pack(
            anchor="w", padx=30)
        self.box_resp = scrolledtext.ScrolledText(
            f, height=13, bg=BG3, fg=GRN,
            font=MONO, relief="flat", insertbackground=WHT)
    ...
    def _tab_cortes(self):
        ...
        self._lbl(f, "Marque os cortes que deseja renderizar.",
                  color=GRY).pack(anchor="w", padx=30)
    ...
    def _tab_render(self):
        ...
        self._lbl(f, "Progresso do render", font=FNTL).pack(
            anchor="w", padx=30, pady=(20,4))
    ...
    def _tab_historico(self):
        ...
        self._lbl(f, "Projetos anteriores", font=FNTL).pack(
            anchor="w", padx=30, pady=(20,4))

        cols = ("ID", "Nome", "Status", "Criado em")
        st = ttk.Style()
        st.configure("Treeview", background=BG3, foreground=WHT,
    ...
    def _start_transcription(self):
        ...
        def run():
            ...
            try:
                res = transcribe(self.video_path, self.v_whisper.get(),
                                 progress_cb=log)
                ...
                def update():
                    ...
                    for s in self.segments:
                        self.box_transcript.insert(
                            "end", f"[{fmt_time(s['start'])}] {s['text']}\n")
                    self.box_prompt.delete("1.0","end")
                    self.box_prompt.insert("1.0", prompt)
                    self._status(
                        f"✅ Transcrição pronta — {len(self.segments)} segmentos. "
                        f"Vá para 🤖 IA Externa.", GRN)
                    self.nb.select(1)
    ...
    def _copy_prompt(self):
        ...
        if not p.strip():
            messagebox.showwarning("Atenção", "Prompt vazio. Transcreva um vídeo primeiro.")
            return
        self.root.clipboard_clear(); self.root.clipboard_append(p)
        messagebox.showinfo("Copiado!",
            "Prompt copiado.\nCole no Claude.ai, ChatGPT ou outra IA.\n"
            "Copie o JSON de resposta e cole na caixa abaixo.")
    ...
    def _draw_cuts(self, cuts: list):
        ...
        for cut in cuts:
            ...
            self._draw_cut_card(cut, cid)

    def _draw_cut_card(self, cut: dict, cid):
        card = tk.Frame(self.cuts_frame, bg=BG3); card.pack(fill="x", pady=3, padx=2)
        ...
        tk.Checkbutton(hdr, variable=var, bg=BG3, fg=WHT,
                       selectcolor=ACC, activebackground=BG3,
                       font=FNTB).pack(side="left")
        dur = cut["end"] - cut["start"]
        tk.Label(hdr, text=cut.get("title","Corte"),
                 bg=BG3, fg=WHT, font=FNTB).pack(side="left", padx=4)
        tk.Label(hdr,
                 text=f"  {fmt_time(cut['start'])} → {fmt_time(cut['end'])}  "
                      f"({fmt_time(dur)})",
                 bg=BG3, fg=GRY, font=FNT).pack(side="left")
        tk.Label(hdr, text=f" {cut.get('archetype','')} ",
                 bg=ACC, fg=WHT, font=("Segoe UI",8,"bold"),
    ...
    def _open_output(self):
        if not self.video_path or not self.project_id: return
        proj    = db.get_project(self.project_id)
        if not proj: return
        safe    = "".join(c for c in proj["name"]
                          if c.isalnum() or c in " _-").strip().replace(" ", "_")
        out_dir = os.path.join(str(Path(self.video_path).parent),
                               f"clipfusion_{safe}")
        if os.path.exists(out_dir):
            os.system(f'xdg-open "{out_dir}"')
        else:
            messagebox.showinfo("Info", f"Pasta ainda não criada:\n{out_dir}")
    ...
    def _load_project(self):
        ...
        if t:
            ...
            ctx = ""
            self.box_prompt.delete("1.0","end")
            self.box_prompt.insert("1.0",
                build_analysis_prompt(self.segments, self.duration, ctx))
        cuts = db.get_cuts(pid)
        if cuts: self._draw_cuts(cuts)
        self.v_name.set(proj["name"])
        self.lbl_video.config(
            text=f"✅ {os.path.basename(proj['video_path'])}", fg=GRN)
        messagebox.showinfo("Carregado",
            f"Projeto '{proj['name']}' carregado.\nStatus: {proj['status']}")
        self.nb.select(0)
    ...
    def _refresh_tree(self):
        ...
        for p in db.list_projects():
            self.tree.insert("","end",
                values=(p["id"], p["name"], p["status"], p["created_at"]))
