"""
ClipFusion — Cut Engine
Corta, adiciona legenda animada, formata por plataforma e aplica anti-copyright.
"""
...
PLATFORM_CONFIGS = {
    "tiktok":    {"w": 1080, "h": 1920, "max_dur": 180, "crf": 20,
                  "preset": "medium", "fps": 30, "abr": "128k", "suffix": "_tiktok"},
    "reels":     {"w": 1080, "h": 1920, "max_dur": 90,  "crf": 21,
                  "preset": "medium", "fps": 30, "abr": "128k", "suffix": "_reels"},
    "shorts":    {"w": 1080, "h": 1920, "max_dur": 60,  "crf": 21,
                  "preset": "medium", "fps": 30, "abr": "128k", "suffix": "_shorts"},
}
...
def render_cut(video_path: str, cut: dict, segments: list,
    ...
    safe_title = "".join(
        ...
        if c.isalnum() or c in " _-").strip().replace(" ", "_")[:40]
    ...
    try:
        ...
        for platform in platforms:
            ...
            w, h = cfg["w"], cfg["h"]
            out_filename = f"{safe_title}{cfg['suffix']}.mp4"
            out_path = os.path.join(output_dir, platform, out_filename)
            os.makedirs(os.path.dirname(out_path), exist_ok=True)

            log(f"  [{platform}] {out_filename} ({fmt_time(dur)})")

            # Filtro escala + legenda
            ...
            sub = f"subtitles='{srt_esc}':force_style='{style}'"
            vf = f"{scale},{sub}" if srt_content.strip() else scale

            # Render sem VA-API primeiro (fallback automático)
            ...
            if r.returncode != 0:
                log(f"  ⚠️ Render falhou: {r.stderr[-100:]}")
                continue
            ...
            if ace_level != "none":
                ...
                cfg_ace = ProtectionConfig.from_level(lvl)
                engine = AntiCopyrightEngine(
                    project_id, idx, cfg_ace, log=log)
